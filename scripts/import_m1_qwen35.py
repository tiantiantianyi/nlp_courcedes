...
def require_complete_from_config(
    annotation_config: dict, *, cli_required: bool
) -> bool:
    allow_missing = annotation_config.get("allow_missing", True)
    if not isinstance(allow_missing, bool):
        raise ValueError("annotation.allow_missing must be a boolean")
    return cli_required or not allow_missing
...
def import_annotations(source: Path, artifacts: Path, *, require_complete: bool = False) -> dict:
    ...
    for line_number, payload in enumerate(source_records, start=1):
...
def main() -> None:
    ...
    require_complete = require_complete_from_config(
        config["annotation"], cli_required=args.require_complete
    )
    report = import_annotations(
        source.resolve(), artifacts, require_complete=require_complete
    )
